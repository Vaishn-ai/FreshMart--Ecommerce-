...
class IsSuperUser(permissions.BasePermission):
    """Only Django superusers may create, edit, or delete catalog data."""
...
class ProductViewSet(viewsets.ModelViewSet):
    """
    list/retrieve are public; write actions require a superuser.
    ...
    def get_permissions(self):
        if self.action in ("create", "update", "partial_update", "destroy", "manage", "manage_detail"):
            return [IsSuperUser()]
    ...
    def manage(self, request):
        """Full catalog for the superuser web manager, including inactive products."""
        qs = Product.objects.all().select_related("category", "brand").prefetch_related("images")
        serializer = ProductListSerializer(qs, many=True, context={"request": request})
        return Response(serializer.data)

    @action(detail=True, methods=["get"], permission_classes=[IsSuperUser], url_path="manage-detail")
    def manage_detail(self, request, pk=None):
        """Full product record for editing from the web manager, including inactive products."""
        product = get_object_or_404(Product.objects.select_related("category", "brand").prefetch_related("images", "variants"), pk=pk)
